...
const processRecurringTransactions = async () => {
  ...
  try {
    ...
    log(
      "info",
      "RecurringTransactions",
      `Found ${recurringTransactions.length} pending recurring transactions`,
    );
    ...
    for (const rt of recurringTransactions) {
      ...
      try {
        ...
        if (
          !lockedRt ||
          !lockedRt.nextExecutionDate ||
          lockedRt.nextExecutionDate > new Date()
        ) {
        ...
        const balanceChange =
          lockedRt.type === "income" ? lockedRt.amount : -lockedRt.amount;
        ...
        if (lockedRt.recurringInterval === "daily") {
          ...
          nextDate = addMonthsClamped(nextDate);
        }
        ...
        log(
          "error",
          "RecurringTransactions",
          `Failed to process transaction ID ${rt._id}`,
          { error: err.message },
        );
      } finally {
        session.endSession();
    ...
    log(
      "info",
      "RecurringTransactions",
      "Completed recurring transactions processing",
      { processedCount, failedCount, durationMs: duration },
    );
  } catch (error) {
    log(
      "error",
      "RecurringTransactions",
      "Critical error in recurring transactions processing",
      { error: error.stack },
    );
  }
};
...
const processBillReminders = async () => {
  ...
  try {
    ...
    log(
      "info",
      "BillReminders",
      `Found ${upcomingSubscriptions.length} active bills in 7-day window`,
    );
    ...
    for (const sub of upcomingSubscriptions) {
      ...
      const prefs = sub.userId.notificationPrefs || {
      ...
      if (dueDate >= targetDateStart && dueDate <= targetDateEnd) {
        try {
          ...
          log(
            "error",
            "BillReminders",
            `Failed to send email to ${sub.userId.email} for ${sub.name}`,
    ...
    const duration = Date.now() - startTime;
    log("info", "BillReminders", "Completed bill reminders processing", {
      emailsSent,
      skipped,
      durationMs: duration,
    });
    ...
    log(
      "error",
      "BillReminders",
      "Critical error in bill reminders processing",
      { error: error.stack },
    );
  }
};
...
const initWorker = () => {
  ...
  log(
    "info",
    "Worker",
    "Background worker initialized, tasks scheduled for 9:00 AM daily.",
  );
};
